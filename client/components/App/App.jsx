...
class App extends Component {
  constructor(props) {
    ...
    this.room = window.location.href.split('?')[1]
  ...
  componentDidMount() {
    ...
    socket.on('userData', ({ socketId, user }) =>
      this.props.getUserData(socketId, user)
    );
  ...
  handleMessageGet = (message) => {
    ...
    if (
      messagesListContainer.clientHeight < messagesListContainer.scrollHeight
    ) {
    ...
    return (
      <div className={styles.App}>
        <div className={styles.AppHeader}>
          ...
          <div className={styles.AppRoom}>dynamic roomz chat</div>
        </div>
        <div className={styles.AppBody}>
          <UsersList users={this.props.users} myId={this.props.user.socketId} />
